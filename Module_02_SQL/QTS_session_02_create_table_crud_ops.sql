...
--------------------------------------------------------------------------- */
DROP DATABASE IF EXISTS day_1; -- Cleanup for re-run
...
DELETE TABLE students; --delete but not reset identity of table

TRUNCATE TABLE students; --delete and reset identity of students table
...
ALTER TABLE students MODIFY COLUMN phone INT DEFAULT(1); -- NULL, NOT NULL, CHECK, .........
-- Rename a column
ALTER TABLE students RENAME COLUMN phone to contact_number;

-- Add Constraint (only applicable for CHECK constraint)
ALTER TABLE students ADD CONSTRAINT age CHECK(age>=18);

-- Drop Constraint
ALTER TABLE students DROP CHECK age;

-- Change column (replace column name with datatype)
...
--------------------------------------------------------------------------- */
DROP DATABASE day_1;
SHOW DATABASES; -- Should not show day_1 anymore
